...
{
    public class PrintDetailMaterialController : Controller
    {
        ...
        public ActionResult GetViewReportQuantitative(int id)
        {
            ...
            ViewBag.PartyType = DA_PARTY_PRODUCT_MATERIAL.Instance.GetPartyType(39);
            TempData["d"] = id;
        ...
        {
            string refer = "";
            return Json((!exportExcel(ref refer, DA_PARTY_PRODUCT_MATERIAL.Instance.GetPartyType(39), DA_Party.Instance.GetById(39))) ? "0!" + refer : "1!/Dinh luong tiec.xlsx");
        }

        public bool exportExcel(ref string refer, SYS_PARTY_TYPE objSYS_PARTY_TYPE, TBL_PARTY objTBL_PARTY)
        {

            List<dynamic> lstDA_PARTY_PRODUCT_MATERIAL = DA_PARTY_PRODUCT_MATERIAL.Instance.GetViewReportForDatatablePagging(0, int.MaxValue, "MaterialName", "", 39);
...

            {
                ...
                try
                {
                    ...
                    wsList.Cells[2, 3].LoadFromText("Tiệc: " + objSYS_PARTY_TYPE.PartyTypeName.ToString());
                    ...
                    try
                    {
                        ...
                        for (int j = 0; j < lstDA_PARTY_PRODUCT_MATERIAL.Count(); j++)
                        {
                            ...
                            wsList.Cells[j + 5, 2].LoadFromText(lstDA_PARTY_PRODUCT_MATERIAL[j].MaterialName);



                            decimal quantity = lstDA_PARTY_PRODUCT_MATERIAL[j].Quantity;
                            string YouWant = quantity.ToString("#,###.0000");
                            wsList.Cells[j + 5, 3].LoadFromText(quantity.ToString());
                            wsList.Cells[j + 5, 3].Style.Numberformat.Format = "#,###.0000";


                            decimal unitPrice = lstDA_PARTY_PRODUCT_MATERIAL[j].UnitPrice;
                            string whatYouWant = unitPrice.ToString("#,##0");
                            wsList.Cells[j + 5, 5].Value = unitPrice.ToString("#,##0");
                            wsList.Cells[j + 5, 5].Style.Numberformat.Format = "#,##0";

                            wsList.Cells[j + 5, 4].LoadFromText(lstDA_PARTY_PRODUCT_MATERIAL[j].UOMName);
                            wsList.Cells[j + 5, 6].LoadFromText(lstDA_PARTY_PRODUCT_MATERIAL[j].VendorName);
