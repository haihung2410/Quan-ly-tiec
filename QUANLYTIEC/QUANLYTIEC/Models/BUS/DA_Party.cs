            ...
            try
            {
                using (var context = (ConnectionEFDataFirst)Activator.CreateInstance(typeof(ConnectionEFDataFirst), _connectionStr))
                {
                    ...
                    getData = (from u in context.TBL_PARTY
                               ...
                               select new { u.PartyID, PartyDate = SqlFunctions.DateName("day", u.PartyDate) + "/" + SqlFunctions.StringConvert((double)u.PartyDate.Month).TrimStart() + "/" + SqlFunctions.DateName("year", u.PartyDate) + " " + SqlFunctions.DateName("hour", u.PartyDate) + ":" + SqlFunctions.DateName("minute", u.PartyDate), u.CustomerName, u.NumberTablePlan, u.NumberTableReal, u.NumberTableVegetarian }).OrderBy((sortColumn == "" && sortColumnDir == "") ? "CustomerName asc" : sortColumn + " " + sortColumnDir).Skip(start).Take(length).ToList<object>();
                    return getData;
                }
            }
            catch (Exception ex)
        ...
        public List<Object> MethodFillDataForDatatablesProductMaterial(int productId, int partyId)
        {
            try
            {
                using (var context = (ConnectionEFDataFirst)Activator.CreateInstance(typeof(ConnectionEFDataFirst), _connectionStr))
                {
                    List<object> getData = new List<object>();
                    //excute query 
                    getData = (from t in ((from n in context.TBL_PARTY_PRODUCT_MATERIAL where n.PartyID == partyId && n.ProductID == productId select n.ID).Count() == 0
                                    ? (from n in context.TBL_PRODUCT_MATERIAL where n.ProductID == productId select new { MaterialID = n.MaterialID, Quantity = n.Quantity, UnitPrice = 0, IsDelivery = false, IdPartyProductMaterial = 0, VendorID = 0 })
                                    : (from n in context.TBL_PARTY_PRODUCT_MATERIAL where n.PartyID == partyId && n.ProductID == productId select new { MaterialID = n.MaterialID, Quantity = n.Quantity, UnitPrice = n.UnitPrice, IsDelivery = n.IsDelivery, IdPartyProductMaterial = n.ID, VendorID = n.VendorID ?? 0 }))
                               ...
                               select new { m.MaterialID, m.MaterialName, t.Quantity, u.UOMName, t.UnitPrice, t.VendorID, t.IsDelivery, t.IdPartyProductMaterial }).ToList<object>();
