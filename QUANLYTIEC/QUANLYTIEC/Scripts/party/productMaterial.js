    ...
    $("#updateButton").click(function () {
        if (confirm("Bạn có đồng ý lấy lại dữ liệu. Mọi thao tác của bạn sẽ không được lưu lại.")) {
            $("form div.x_content").each(function (index) {
                ...
                addItemForDatatable(productId, true, arrayTable[index], index, dataSelect);
            });
        }
    });
        ...
        success: function (arrayDatatable) {
            if (arrayDatatable != "null") {
                ...
                $.each(arrayDatatable, function (index_dataTable, dataTable) {
                    ...
                    addItemForDatatable(dataTable.ProductID, false, table, 0, dataSelect);
...
function addItemForDatatable(productId, tableIsExist, currentTable, indexTable, dataSelect) {
    if (tableIsExist)
